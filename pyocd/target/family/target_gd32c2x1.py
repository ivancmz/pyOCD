...
_OB_SAFE_DEFAULTS = {
    "spc":          SPC_UNLOCKED,
    "user":         0x3f6f7f,   # BOR enabled 2.8/2.9 V, WWDG/FWDG software, boot from flash
    "wp0_saddr":    0x3f,
    "wp0_eaddr":    0x00,
    "wp1_saddr":    0x3f,
    "wp1_eaddr":    0x00,
    ...
    "dcrp1_saddr":  0x7f,
    "dcrp1_eaddr":  0x00,
    "dcrp_eren":    0xff,       # mandatory per RM when no DCRP area is defined
    "scr_page_cnt": 0x00,
    "scr_bootlk":   0x00,
}

# When True, all OB fields except SPC are taken from _OB_SAFE_DEFAULTS when
# writing option bytes, ignoring whatever values were read from the device.
# When False, the values read from the device are preserved (original behaviour).
_OB_WRITE_USES_SAFE_DEFAULTS = True

FLASH_TIMEOUT = 5.0
OB_PROGRAM_TIMEOUT = 20.0  # unlock from SPC=0xBB triggers mass erase of user flash (~10s on 64KB)
...
class GD32C2x1(CoreSightTarget):
    ...
    def _set_security_state(self, spc_value):
        ...
        for attempt in range(1, SECURITY_STATE_RETRIES + 1):
            try:
                ...
                if ob["spc"] == 0xFF:
                    # OBs are in OBERR state (complement bytes mismatched after reset reload).
                    # Always do a recovery write with known-safe values to repair the complement
                    # bytes — do NOT early-return, even if the default spc would already match.
                    LOG.warning("%s: option bytes corrupted (SPC=0xFF); restoring with safe defaults",
                                GD32C2X1_TAG)
                    ob = dict(_OB_SAFE_DEFAULTS)
                elif ob["spc"] == spc_value:
                    return True

                if _OB_WRITE_USES_SAFE_DEFAULTS:
                    ob_write = dict(_OB_SAFE_DEFAULTS)
                    ob_write["spc"] = spc_value
                else:
                    ob["spc"] = spc_value
                    ob_write = ob
                LOG.info("%s: writing option bytes for spc=0x%02x",
                            GD32C2X1_TAG, ob_write["spc"])
                self._write_option_bytes(ob_write)
    ...
    def _write_ob_launch(self):
        """Write OBLAUNCH with AP-wait retry.

        For lock (~5 ms, OB sector only): the FMC queues OBLAUNCH behind the running
        OBSTART; the brief AHB stall is within the STLink internal retry window so the
        first write attempt succeeds immediately.
        For unlock from SPC=0xBB (mass erase ~10 s): the AHB stall exceeds the STLink
        timeout and every write attempt raises TransferError/TargetError (AP wait).
        We catch those errors and retry until the mass erase completes and the write
        is accepted.
        """
        deadline = time.time() + OB_PROGRAM_TIMEOUT
        while time.time() < deadline:
            try:
                self._mini_write32(FMC_CTL, FMC_CTL_OBLAUNCH)
                return  # write accepted
            except (exceptions.TransferError, exceptions.TargetError):
                pass  # AHB stalled during mass erase — expected, retry
            time.sleep(0.05)
        raise exceptions.TimeoutError("timed out writing OBLAUNCH to GD32C2x1")
    ...
    def _write_option_bytes(self, ob):
        ...
        self._mini_write32(FMC_WP0, (ob["wp0_eaddr"] << 16) | ob["wp0_saddr"])
        self._mini_write32(FMC_WP1, (ob["wp1_eaddr"] << 16) | ob["wp1_saddr"])
        self._mini_write32(FMC_DCRP0, ob["dcrp0_saddr"])
        self._mini_write32(FMC_DCRP1, (ob["dcrp_eren"] << 24) | ob["dcrp0_eaddr"])
